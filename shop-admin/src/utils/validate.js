/**
 * @Author: fremember
 * @Date: 2023-08-26 23:17:44
 * @LastEditTime: 2023-08-26 23:50:25
 * @LastEditors: fremember
 * @Description: 全局使用的校验方法
 * @FilePath: /fremember_github/vueDemos/shop-admin/src/utils/validate.js
 */

/**
 * 校验输入内容是不是在可选范围
 * @param {*} len number 内容的长度
 * @param {*} min number 长度的下限
 * @param {*} max number 长度的上限
 * @return boolean
 */
export function isLimit (len, min, max) {
    return len <= max && len >= min
}

/**
 * 校验密码的安全强度，必须包含数字、特殊字符和英文字母的大小写，且长度至少6位
 * @param {*} pwd 密码明文
 * @returns 
 */
export function pwdStrength (pwd) {
    return /^(?=.*[0-9])(?=.*[!@#$%^&*])(?=.*[a-z])(?=.*[A-Z]).{6,}$/.test(pwd)
}
...
export function isPhone (phone) {
    return /^1[356789]\d{9}/.test(phone)
}
