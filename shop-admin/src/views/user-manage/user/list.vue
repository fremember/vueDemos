        ...
        <form-drawer ref="formDrawerRef" :title="drawerTitle" @submit="handleSubmit">
            <el-form :model="form" ref="formRef" :rules="rules" label-width="80px" :inline="false">
                ...
                <el-form-item label="会员等级" prop="user_level_id">
                    <el-select v-model="form.user_level_id">
                        <el-option
                            v-for="item in user_level"
                            :key="item.id"
                            :label="item.name"
...
<script setup>
    ...
    const check = {
        username: (rule, value, callback) => {
            if(!isLimit(value.length, 4, 25)) {
                callback(new Error('用户名长度必须在4～25之间'))
            } else {
                callback()
            }
        },
        password: (rule, value, callback) => {
            ...
            } else if(!pwdStrength(value)) {
                callback(new Error('密码必须包含数字、特殊字符和英文字母的大小写'))
            } else {
                callback()
            }
        },
        phone: (rule, value, callback) => {
            if(!value || (value && isPhone(value))) {
                callback()
