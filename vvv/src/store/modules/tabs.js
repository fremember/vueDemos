...
let state = {
    ...
    mutations = {
        ...
        clearTabs(state) {
            state.tabs = state.tabs.filter(item => item.fullPath == '/dashboard/index')
            router.push({ path: '/dashboard/index' })
        },
        clearOtherTabs(state, tab) {
            state.tabs = state.tabs.filter(item => item.fullPath == '/dashboard/index' || item.fullPath == tab.fullPath)
        }
    },
    actions = {
